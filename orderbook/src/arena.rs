...
impl OrderArena {
    pub fn new(capacity: usize) -> Self {
        ...
        // Preallocate
        for i in 0..capacity {
            list.orders.push(Order::LimitOrder {
                id: 0,
                price: 0,
                qty: 0,
            });
    ...
    pub fn insert(&mut self, id: u128, price: u64, qty: u64) -> usize {
        match self.free.pop() {
            None => {
                self.orders.push(Order::LimitOrder { id, price, qty });
...
impl Index<usize> for OrderArena {
    type Output = Order::LimitOrder;

    #[inline]
    fn index(&self, index: usize) -> &Order::LimitOrder {
        &self.orders[index]
    }
}
...
impl IndexMut<usize> for OrderArena {
    #[inline]
    fn index_mut(&mut self, index: usize) -> &mut Order::LimitOrder {
